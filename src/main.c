...
bool isWAV(FILINFO fileInfo) {
	int i = 0;
	for (i = 0; i < 13; i++) {
		file_name[i] = (char) fileInfo.fname[i];
		...
		if (fileInfo.fname[i] == '.') {
			if (fileInfo.fname[i + 1] == 'W' && fileInfo.fname[i + 2] == 'A'
				...
				TM_HD44780_Clear();
				WS2812B_SetRGB(0, 1, 0, 0, 255);
				TM_HD44780_Puts(2, 0, file_name);
				return 1;
			}
		}
	}
...
int main(void) {
	...
	uint8_t customChar[] = { 0x1F,	// xxx 11111
			0x11,	// xxx 10001
			0x11,	// xxx 10001
			0x11,	// xxx 10001
			0x11,	// xxx 10001
	...
	TM_HD44780_Clear();
	char discoplayer[12] = "DISCO PLAYER";
	TM_HD44780_Puts(2, 0, discoplayer);
	Delayms(3000);
